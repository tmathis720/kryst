...
mod real {
    ...
    use kryst::matrix::preprocess::{
        ...
        nonsymmetric_max_transversal_permutation, permute_rows_cols, row_1_norm_scaling,
    ...
    fn parse_reference_mode_arg(
        args: &[String],
    ) -> Result<ReferenceMode, Box<dyn std::error::Error>> {
        let mut idx = 0usize;
        while idx < args.len() {
    ...
    fn parse_mpi_pc_stage_arg(
        args: &[String],
    ) -> Result<MpiPcStageMode, Box<dyn std::error::Error>> {
        let mut idx = 0usize;
        while idx < args.len() {
        ...
        if any_fail {
            if rank == 0 {
                if let Err(err) = &result {
                    ...
                    eprintln!(
                        "Collective failure in {phase_name}: another rank reported an error."
                    );
                }
            }
            comm.barrier();
    ...
    pub fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if preprocess_cfg.matching_perm {
            let (row_perm, col_perm) =
                nonsymmetric_max_transversal_permutation(&transformed_matrix);
        ...
        if rank == 0 {
            ...
            if inspection.zero_diagonal_rows > 0 && !user_pc {
                println!(
                    "  Default iterative PC policy: using 'none' (zero diagonal rows detected)."
                );
            }
            if !inspection.is_likely_symmetric && !user_solver {
                println!(
                    "  Default iterative solver policy: using 'gmres' (nonsymmetric estimate)."
                );
            }
        ...
        let (stats, solution, convergence_history, setup_time, solve_time) =
            collective_phase_result(
                "iterative",
                run_phase(
                    "iterative",
                    &iterative_solver,
        ...
        if rank == 0 {
            ...
            match output_status {
                ...
                WrittenOutputVerificationStatus::Marginal => {
                    println!(
                        "⚠ written output is only marginally acceptable on this configuration."
                    );
                }
                WrittenOutputVerificationStatus::Failed => {
        ...
        let iterative_path_success = stats.reason.is_converged()
