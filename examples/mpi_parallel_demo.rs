use kryst::*;
use kryst::parallel::Comm;  // Import the trait
...
fn example_basic_comm_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Expected: {} (local_value * comm_size)", local_value * comm.size() as f64);
...
fn example_parallel_inner_products() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Expected global dot product: {} (total across {} processes)", 
             10.0 * comm.size() as f64, comm.size());
...
fn example_communicator_splitting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Sub-communicator (color {}) - Rank: {}, Size: {}", 
             color, sub_comm.rank(), sub_comm.size());
...
/// Example of how KspContext would be used with communicators (once fully implemented)
#[allow(dead_code)]
fn example_ksp_with_comm() -> Result<(), Box<dyn std::error::Error>> {
    use faer::Mat;
    
    println!("\n4. KSP Context with Communicator (Conceptual)");
    println!("----------------------------------------------");
    ...
    for i in 0..n {
        a[(i, i)] = 2.0;  // Diagonal matrix
        if i > 0 { a[(i, i-1)] = -1.0; }  // Sub-diagonal
    ...
    // Create communicator
    #[cfg(not(any(feature="mpi", feature="rayon")))]
    let comm = parallel::UniverseComm::Serial;
    #[cfg(all(feature="rayon", not(feature="mpi")))]
    ...
    ksp.setup_with_comm(&a, n, comm)?;
    
    println!("✓ KspContext setup with communicator successful");
    println!("  Matrix size: {}x{}", n, n);
    println!("  Communicator size: {}", 
             match &ksp.comm {
                 Some(c) => c.size(),
                 None => 1,
             });
    
    // Note: Actual solve would require updating all solver implementations
    // to pass the communicator to InnerProduct methods
