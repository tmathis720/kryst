...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let args = vec![
        ...
        "-pc_asm_inner_pc", "ilutp",
    ];
    let pc_opts = PcOptions::from_args(&args)?;
    println!("✓ ASM options parsed: overlap={:?}, subdomains={:?}, inner_pc={:?}", 
    ...
    let args = vec![
        ...
        "-pc_chebyshev_lambda_max", "2.0",
    ];
    let pc_opts = PcOptions::from_args(&args)?;
    println!("✓ Chebyshev options parsed: lambda_min={:?}, lambda_max={:?}", 
    ...
    let pc_opts = PcOptions::from_args(&args)?;
    println!("✓ AMG options parsed: levels={:?}, strength_threshold={:?}", 
